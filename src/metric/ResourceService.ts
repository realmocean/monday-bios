...
export class ResourceService {
    ...
    getResourceEntryByOrigin(origin: string, resources: PerformanceResourceTiming[]): any {
        ...
        var entry = {
            ...
            totalTimeline: this.getTimeRangesForResources(resources).join(",")
        };
    ...
    getAllThirdPartyJs(): any[] {
        ...
        var allScripts = allResources.filter(resource => {
            if (!resource || !this.isScript(resource)) {
                return false;
            }
            if(!this.isThirdParty(resource.name)) {
                return false;
            }
            var resourceKey = resource.name;
            if (this.thirdPartyCache[resourceKey]) {
                return false;
            }
            this.thirdPartyCache[resourceKey] = true;

            return true;
        }).map(jsResource => {
            return {
                url: jsResource.name,
                startedOn: roundToDecimal(jsResource.startTime),
                duration: roundToDecimal(jsResource.duration)
            }
        });
